...
def check_contributing_file_recursive(api_url, username, token):
    """
    Recursively check for CONTRIBUTING.md or CONTRIBUTING.rst in a repository's directory tree.
    """
    try:
        response = requests.get(api_url, auth=(username, token))
        if response.status_code == 403:
            handle_rate_limit(response)
            return check_contributing_file_recursive(api_url, username, token)
        ...
        for item in response.json():
            ...
            if item['type'] == 'file' and item['name'] in ['CONTRIBUTING.md', 'CONTRIBUTING.rst']:
                return True

            # Recursively check subdirectories
            if item['type'] == 'dir':
                subdir_url = item['url']
                if check_contributing_file_recursive(subdir_url, username, token):
                    return True
...
def check_contributing_file(repo_url, username, token):
    """
    Check if the repository contains CONTRIBUTING.md or CONTRIBUTING.rst file in the root or subdirectories.
    """
...
def process_csv(input_file, output_file, username, token):
    ...
    try:
        ...
        # Add a new column for CONTRIBUTING file presence
        df['contributing_present'] = False
        for index, row in df.iterrows():
            repo_url = row.get('html_url', None)
            df.at[index, 'contributing_present'] = check_contributing_file(repo_url, username, token)
...
def main():
    parser = argparse.ArgumentParser(description="Check for CONTRIBUTING files in GitHub repositories.")
